...
function openai_post_json(string $url, array $payload): array {
  $key = openai_key();
  if ($key === '') return ['ok'=>false,'error'=>'Missing OpenAI API key in config.php (openai.api_key).'];
  ...
  if ($out === false) return ['ok'=>false,'error'=>"cURL error: $err"];
...
function openai_answer(string $userText, string $lang='AUTO'): array {
  ...
  if ($block) return ['ok'=>true,'text'=>$block];
...
function openai_tts_mp3(string $text, ?string $voice=null, ?string $model=null): array {
  ...
  if ($bin === false) return ['ok'=>false,'error'=>"cURL error: $err"];
  if ($code < 200 || $code >= 300) {
